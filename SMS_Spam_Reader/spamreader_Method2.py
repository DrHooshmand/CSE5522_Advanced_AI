    ...
    for (tag,elem) in  test_items:
        ...
        for word in acc_words:
            if word in elem:
                ...
                # p_E_giv_spam = p_E_giv_spam * eps if cond_spam[acc_words.index(word)] == 1 else p_E_giv_spam * (1 - cond_spam[acc_words.index(word)])
                # p_E_giv_ham =  p_E_giv_ham * eps  if cond_ham[acc_words.index(word)] ==1 else p_E_giv_ham * (1 - cond_ham[acc_words.index(word)] )
                p_E_giv_spam =  float ( p_E_giv_spam * (1 - cond_spam[acc_words.index(word)]) )
                p_E_giv_ham =   float( p_E_giv_ham * (1 - cond_ham[acc_words.index(word)] ) )
            #print ((p_E_giv_spam,p_E_giv_ham,cond_spam[acc_words.index(word)],cond_ham[acc_words.index(word)]))
            # if p_E_giv_ham == p_E_giv_spam:
            #     print("fault")

                #p_w_mult = p_w_mult * (w_ham[acc_words.index(word)] + w_spam[acc_words.index(word)]) / len(acc_words)
                # p_w_mult = 1

        nom_ham = (p_E_giv_ham * p_ham )
        nom_spam = (p_E_giv_spam * p_spam)
        # Control for the cases that there is a word always been seen in spam training AND another word always been seen in ham ....> Prints fault and categorizes as Spam
        # With applying the correction in Bonus part, this never happens! (never see "fault" printed)
        if nom_ham == nom_spam and nom_ham == 0:
            print("fault")
            P_ham_giv_E = 0
            P_spam_giv_E = 1

        else:
            P_ham_giv_E = nom_ham / ( nom_ham + nom_spam )
            P_spam_giv_E = nom_spam / ( nom_ham + nom_spam )
